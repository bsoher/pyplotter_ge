...
# Beware -- lots of other modules may import this one. If you import
...
def get_data_dir():
    ...
    value returned by wx.GetAppName(). This call always uses "pyplotter_ge".

    You can expect data dir names something like this:
      * Windows: C:\Users\%USERNAME%\AppData\Local\PyPlotter_GE
      * Linux:   ~/.PyPlotter_GE
      * OS X:    ~/Library/Application Support/PyPlotter_GE
    """
    ...
    if platform == "linux":
        ...
        if not os.path.exists(data_dir):
            ...
            # ~/.PyPlotter_GE. To be better citizens, we'll now begin to respect the
            # new-ish XDG standard described on freedesktop.org. So on machine
            # where ~/.PyPlotter_GE doesn't yet exist (i.e. new installs), we'll try
...
def get_pyplotter_ge_install_directory():
    """Returns the fully-qualified name of the directory in which pyplotter_ge has
    been installed.

    Under OS X this will return something like --
    /Library/Frameworks/Python.framework/Versions/3.9/lib/python3.9/site-packages/pyplotter_ge

    Under Linux this will return something like --
    /usr/local/lib/python3.9/dist-packages/pyplotter_ge

    Under Windows this will return something like --
    c:\\Python39\\lib\\site-packages\\pyplotter_ge
    """
    # We figure out the path by picking a file at a known location in the
    # pyplotter_ge hierarchy, getting that file's path and inferring the rest.
    path = ""

    if hasattr(sys, "frozen") and sys.frozen:
        # Looks like this is being used in a PyInststaller bundle. This never
        # happens in pyplotter_ge, only when this module is used in the Siemens
        # realtime project.
#        path = os.environ['_MEIPASS2']
        path = sys._MEIPASS
    else:
        # This is the normal case in pyplotter_ge.

        # Strange but true -- we can 'import pyplotter_ge', although it's meaningless
        # to do so in a practical sense. However, we can then use pyplotter_ge.__file__
        # to see where that module resides.

        import pyplotter_ge
        path = os.path.abspath(pyplotter_ge.__file__)
...
def get_pyplotter_ge_version():
    """Returns pyplotter_ge's version as a string, e.g. "1.0.1"."""
    path = os.path.join(get_pyplotter_ge_install_directory(), '..', 'VERSION')

    if os.path.exists(path):
        # The VERSION file isn't distributed as part of pyplotter_ge, so if I can see it, this
        # must be a development install.
        version = open(path).read().strip()
    else:
        # This is an end-user installation.
        version = pkg_resources.get_distribution('pyplotter_ge').version
